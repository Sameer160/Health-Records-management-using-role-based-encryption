 ...
 * @author mvinoth
 */
@WebServlet(urlPatterns = {"/Provider_Login"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            DbConnection db=new DbConnection();
                ResultSet rs=db.Select("select * from provider_details where UP_Name='"+name+"' and P_Pass='"+pass+"'");
                if(rs.next())
                {
                     String id=rs.getString("P_Name");
                     session.setAttribute("msg","Successfully Login");
                     session.setAttribute("P_Id",id);
                     session.setAttribute("provider",rs.getString("P_Name"));
                     session.setAttribute("provider1",rs.getString("H_Name"));
